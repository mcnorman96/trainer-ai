...
export async function POST(request: NextRequest, context: { params: Promise<{ moduleId: string }> }) {
  ...
  if (!moduleId) {
    return NextResponse.json({ success: false, error: 'Missing moduleId' }, { status: 400 });
  }
  ...
  if (lessons.length > 0 && lessons.every(l => l.completed)) {
    ...
    return NextResponse.json({ success: true, moduleCompleted: true });
  }
  return NextResponse.json({ success: true, moduleCompleted: false });
}
